...
# DomMate 本地Docker构建和测试脚本（使用sudo）

set -e

echo "🧪 ================================="
echo "🚀 DomMate 本地Docker构建测试 (sudo版本)"
echo "🧪 ================================="
...
# 显示构建进度
sudo docker build -t $IMAGE_NAME . --progress=plain
...
echo ""
echo "📦 检查前端构建产物..."
if sudo docker exec $CONTAINER_NAME ls -la /app/dist/index.html > /dev/null 2>&1; then
    echo "✅ 前端构建产物存在"
    frontend_files=$(sudo docker exec $CONTAINER_NAME ls /app/dist/ | wc -l)
...
echo ""
echo "💾 检查数据持久化..."
if sudo docker exec $CONTAINER_NAME ls -la /app/data/ > /dev/null 2>&1; then
    echo "✅ 数据目录存在"
    echo "📁 数据目录内容："
    sudo docker exec $CONTAINER_NAME ls -la /app/data/
else
    echo "❌ 数据目录不存在"
fi
...
echo ""
echo "💽 数据卷状态："
sudo docker volume inspect $VOLUME_NAME --format "{{.Name}}: {{.Mountpoint}}"
...
echo "   查看实时日志: sudo docker logs -f $CONTAINER_NAME"
echo "   进入容器: sudo docker exec -it $CONTAINER_NAME sh"
echo "   检查进程: sudo docker exec $CONTAINER_NAME ps aux"
echo "   检查端口: sudo docker exec $CONTAINER_NAME netstat -tulpn"
...
if [[ $REPLY =~ ^[Yy]$ ]]; then
    echo "✅ 容器将继续运行，可以手动测试"
    echo "📋 手动停止命令: sudo docker stop $CONTAINER_NAME"
    echo "🧹 完全清理命令: sudo docker stop $CONTAINER_NAME && sudo docker rm $CONTAINER_NAME && sudo docker volume rm $VOLUME_NAME"
